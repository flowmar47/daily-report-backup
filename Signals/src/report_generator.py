        ...
        for pair, signal in sorted_signals:
            if signal.action != 'HOLD':
                lines.extend([
                    ...
                    f"  Target: {self._format_price(pair, signal.exit_price)} ({signal.target_pips:.0f if signal.target_pips else 'N/A'} pips)",
    ...
    def _format_signal_text(self, pair: str, signal: TradingSignal) -> List[str]:
        ...
        if signal.action != 'HOLD':
            ...
            lines.extend([
                ...
                f"  Stop Loss:       {self._format_price(pair, signal.stop_loss)}",
                f"  Target Pips:     {signal.target_pips:.0f if signal.target_pips else 'N/A'} pips",
                f"  Days to Target:  {signal.days_to_target if signal.days_to_target else 'N/A'} days",
                ""
            ])

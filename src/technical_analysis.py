...
class TechnicalAnalyzer:
    ...
    def _detect_harami(self, df: pd.DataFrame) -> List[CandlestickPattern]:
        ...
        for i in range(1, len(df)):
            ...
            if (prev_body > curr_body * 1.5 and  # Previous body significantly larger
                ...
                strength = (prev_body / curr_body) / 10.0  # Normalized strength
    ...
    def get_comprehensive_technical_analysis(self, pair: str) -> Dict[str, Any]:
        ...
        Multi-timeframe analysis: 30min, 1hour, 4hour for increased signal frequency
        Focuses on 4H candlestick patterns as primary signal
        """
        # Fetch multi-timeframe forex data
        forex_30m = data_fetcher.fetch_forex_data(pair, '30min')
        forex_1h = data_fetcher.fetch_forex_data(pair, '1hour')
        forex_4h = data_fetcher.fetch_forex_data(pair, '4hour')
        forex_daily = data_fetcher.fetch_forex_data(pair, 'daily')
        
        if not forex_4h:
            return {'error': f'No 4H data available for {pair}'}
        
        # At least one timeframe required for analysis
        available_timeframes = []
        if forex_30m: available_timeframes.append('30min')
        if forex_1h: available_timeframes.append('1hour')
        if forex_4h: available_timeframes.append('4hour')
        ...
        # Multi-timeframe Candlestick patterns
        # 4H patterns (CRITICAL - highest weight)
        patterns_4h = self.analyze_4h_candlesticks(forex_4h)
        analysis_result['candlestick_patterns_4h'] = [
            {
                'name': p.name,
            ...
            analysis_result['candlestick_patterns_1h'] = [
                {
                    'name': p.name,
                    'signal_type': p.signal_type,
                    'strength': p.strength * 0.7,  # Reduced weight for shorter timeframe
                    'timestamp': p.timestamp,
        ...
        # 30M patterns (lowest weight for quick signals)
        if forex_30m:
            patterns_30m = self.analyze_4h_candlesticks(forex_30m)
            analysis_result['candlestick_patterns_30m'] = [
                {
                    'name': p.name,
                    'signal_type': p.signal_type,
                    'strength': p.strength * 0.5,  # Further reduced weight
                    'timestamp': p.timestamp,
                    'description': f"30M {p.description}",
                    'timeframe': '30min'
                } for p in patterns_30m
            ]
        ...
        # Define timeframes and their data
        timeframe_data = {}
        if forex_30m:
            timeframe_data['30min'] = {
                'prices': [item['close'] for item in forex_30m['data']],
                'highs': [item['high'] for item in forex_30m['data']],
                'lows': [item['low'] for item in forex_30m['data']]
            }
        if forex_1h:
        ...
        if forex_4h:
            timeframe_data['4hour'] = {
                'prices': [item['close'] for item in forex_4h['data']],
                'highs': [item['high'] for item in forex_4h['data']],
                'lows': [item['low'] for item in forex_4h['data']]
            }
        if forex_daily:
        ...
        # Calculate Average Weekly Range for target setting using 4H data
        if forex_4h and '4hour' in timeframe_data:
            highs_4h = timeframe_data['4hour']['highs']
            lows_4h = timeframe_data['4hour']['lows']
            
            if len(highs_4h) >= 42:  # 42 4H candles = 1 week
                weekly_ranges = []
                for i in range(0, len(highs_4h) - 42, 42):
                    week_high = max(highs_4h[i:i+42])
                    week_low = min(lows_4h[i:i+42])
        ...
        analysis_result['signal_summary'] = self._generate_signal_summary(patterns_4h, indicators, short_term_signals)
